...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    (this.includeDem('street') ? '<a data-count="street">乡镇</a>' : '') +
                    (this.includeDem('village') ? '<a data-count="village">村</a>' : '') +
                    '</div>' +
        ...
        defineDems: function () {
            var stop = false;
            $.each([PROVINCE, CITY, DISTRICT,STREET,VILLAGE], $.proxy(function (i, type) {
                if (!stop) {
                    this.dems.push(type);
                }
        ...
        feedVal: function () {
            ...
            if (! typeof searchSite === 'undefined') {
                //回调方法，引用页面定义
                if (searchSite instanceof Function) {
                    searchSite(this.$element.attr('id'),this.getVal());
                }
            }
        },
        ...
        output: function (type) {
            ...
            value = (item ? item.code : null) || options[type].split('/')[0];
                    ...
                    type === CITY ? this.$province && this.$province.find('.active').data('code') :
                        type === DISTRICT ? this.$city && this.$city.find('.active').data('code') :
                            type === STREET ? this.$district && this.$district.find('.active').data('code') :
                                type === VILLAGE ? this.$street && this.$street.find('.active').data('code') : code
            );
            ...
            if(code != undefined && code != '86'){
                $.ajax({
                    type: "POST",
                    url: basePath+"/consumer/basedata/region/getregionlist",
                    data: {regionId:code},
                    dataType: "json",
                    async: false,
                    success: function (dto) {
                        if(dto.success){
                            ...
                            $.each(data, function (i, item) {
                                searchDatas.push({code:item.regionId,address:item.regionName});
                            })
                        }else{
                            alert(dto.message);
                        }
            ...
            if(searchDatas.length > 0){
                $.each(searchDatas,function (index,item) {
                    if (item.code === value) {
                        matched = {
                            code: item.code,
        ...
        customerize:function(){
            var options = this.options;
            var text = '';
            //将tab中对应数据选中，做两次each是因为，在第二个each中this不再代表当前对象，而是type，无法调用output()方法
            //this是html元素对象 $(this)成为jquery对象，包装后 $(this) 就会继承 jQuery 的方法。 本质就是JavaScript与jQuery对象的转换
            $.each(this.dems, $.proxy(function (i, type) {
                var $select = this['$' + type];
                if(options[type].indexOf('undefined') == -1) {
                    var code = options[type].split('/')[0] ;
                    var address = options[type].split('/')[1];
                    $select.data('item', {address: address , code: code});
                }else{
                    $select.data('item',null);
                }
                this.output(type);
            ...
            this.tab(PROVINCE);
            //给textspan赋值，即页面上的显示值
            $.each(this.dems,function (index,type) {
                if(options[type].indexOf('undefined') == -1) {
                    var code = options[type].split('/')[0] ;
                    var address = options[type].split('/')[1];
                    text += (type == 'province' ? '' : '/') + '<span class="select-item" data-count="' +
                        type + '" data-code="' + code + '">' + address + '</span>';
                }
            });
            if (text) {
                this.$textspan.find('>.placeholder').hide();
    ...
    $.fn.citypicker = function (option) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                ...
                //isPlainObject()判断一个对象是否为空
                options = $.extend({}, $this.data(), $.isPlainObject(option) && option);
                $this.data(NAMESPACE, (data = new CityPicker(this, options)));
            }else{
                if(data.options && option && typeof option.action === 'string' && $.isFunction(fn = data[option.action])){
                    var old = data.options;
                    old.province = option.province;
                    old.city = option.city;
                    old.district = option.district;
                    old.street = option.street;
                    old.village = option.village;
                    old.action = option.action;

                    //执行customerize方法，即给一个存在的citypicker重新赋值
                    fn.apply(data, args);
                }
            }
